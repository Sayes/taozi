    ...
    <div class="header__left">
      ...
      <div class="group__tabs">
        <a-tabs :activeKey="activeKey" @tabClick="tabClick">
          <a-tab-pane :style="{ color: '#FEFEFE' }" v-for="item in routers" :key="item.id" :tab="item.name" />
        </a-tabs>
      </div>
    </div>
    <div class="header__right">
      <a-dropdown :trigger="['click', 'hover']">
        <div class="header__avatar">
          ...
          <div class="header__avatar-name">{{ displayName.length > 0 ? displayName : 'admin' }}</div>
        </div>
        <template #overlay>
          <a-menu>
...
export default defineComponent({
  ...
  setup() {
    ...
    const tabClick = (e: number) => {
      ...
      if ((<any>foundItem)?.redirect) {

        router.push((<any>foundItem).redirect);
      }
